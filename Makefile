...
IMAGE_NAME := runstarship/starship
...
build-service: ## Build the main Go service
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -v -o starship .
...
dev-docker:
	GOOS=linux GOARCH=amd64 go build -o starship .
	docker build -f Dockerfile.dev -t starship-dev .
...
release: ## Create packages for a release
	docker run -v $$(pwd):/go/src/github.com/runstarship/starship circleci/golang:1.17 sh -c 'cd /go/src/github.com/runstarship/starship && scripts/binary-release.sh'

// Copyright 2017 starship Media Inc.
//
...
import (
	...
	"github.com/runstarship/starship/server"
	"github.com/runstarship/starship/server/events/vcs/bitbucketcloud"
	"github.com/runstarship/starship/server/events/yaml/valid"
	"github.com/runstarship/starship/server/logging"
...
const (
	...
	DefaultVCSStatusName    = "starship"
	DefaultWebBasicAuth     = false
	DefaultWebUsername      = "starship"
	DefaultWebPassword      = "starship"
)

var stringFlags = map[string]stringFlag{
	ADTokenFlag: {
		description: "Azure DevOps token of API user. Can also be specified via the starship_AZUREDEVOPS_TOKEN environment variable.",
	},
	ADUserFlag: {
	...
	ADWebhookPasswordFlag: {
		description: "Azure DevOps basic HTTP authentication password for inbound webhooks " +
			...
			" SECURITY WARNING: If not specified, starship won't be able to validate that the incoming webhook call came from your Azure DevOps org. " +
			"This means that an attacker could spoof calls to starship and cause it to perform malicious actions. " +
			"Should be specified via the starship_AZUREDEVOPS_WEBHOOK_PASSWORD environment variable.",
		defaultValue: "",
	},
	ADWebhookUserFlag: {
	...
	starshipURLFlag: {
		description: "URL that starship can be reached at. Defaults to http://$(hostname):$port where $port is from --" + PortFlag + ". Supports a base path ex. https://example.com/basepath.",
	},
	AutoplanFileListFlag: {
		description: "Comma separated list of file patterns that starship will use to check if a directory contains modified files that should trigger project planning." +
	...
	BitbucketTokenFlag: {
		description: "Bitbucket app password of API user. Can also be specified via the starship_BITBUCKET_TOKEN environment variable.",
	},
	...
	BitbucketWebhookSecretFlag: {
		description: "Secret used to validate Bitbucket webhooks. Only Bitbucket Server supports webhook secrets." +
			" SECURITY WARNING: If not specified, starship won't be able to validate that the incoming webhook call came from Bitbucket. " +
			"This means that an attacker could spoof calls to starship and cause it to perform malicious actions. " +
			"Should be specified via the starship_BITBUCKET_WEBHOOK_SECRET environment variable.",
	},
	CheckoutStrategyFlag: {
		description: "How to check out pull requests. Accepts either 'branch' (default) or 'merge'." +
			" If set to branch, starship will check out the source branch of the pull request." +
			" If set to merge, starship will check out the destination branch of the pull request (ex. master)" +
			" and then locally perform a git merge of the source branch." +
			" This effectively means starship operates on the repo as it will look" +
	...
	DataDirFlag: {
		description:  "Path to directory to store starship data.",
	...
	GHTokenFlag: {
		description: "GitHub token of API user. Can also be specified via the starship_GH_TOKEN environment variable.",
	},
	GHAppKeyFlag: {
	...
	GHOrganizationFlag: {
		description:  "The name of the GitHub organization to use during the creation of a Github App for starship",
		defaultValue: "",
	},
	GHWebhookSecretFlag: {
		description: "Secret used to validate GitHub webhooks (see https://developer.github.com/webhooks/securing/)." +
			" SECURITY WARNING: If not specified, starship won't be able to validate that the incoming webhook call came from GitHub. " +
			"This means that an attacker could spoof calls to starship and cause it to perform malicious actions. " +
			"Should be specified via the starship_GH_WEBHOOK_SECRET environment variable.",
	},
	GitlabHostnameFlag: {
	...
	GitlabTokenFlag: {
		description: "GitLab token of API user. Can also be specified via the starship_GITLAB_TOKEN environment variable.",
	},
	GitlabWebhookSecretFlag: {
		description: "Optional secret used to validate GitLab webhooks." +
			" SECURITY WARNING: If not specified, starship won't be able to validate that the incoming webhook call came from GitLab. " +
			"This means that an attacker could spoof calls to starship and cause it to perform malicious actions. " +
			"Should be specified via the starship_GITLAB_WEBHOOK_SECRET environment variable.",
	},
	LogLevelFlag: {
	...
	RepoConfigFlag: {
		description: "Path to a repo config file, used to customize how starship runs on each repo. See runstarship.io/docs for more details.",
	},
	RepoConfigJSONFlag: {
		description: "Specify repo config as a JSON string. Useful if you don't want to write a config file to disk.",
	},
	RepoAllowlistFlag: {
		description: "Comma separated list of repositories that starship will operate on. " +
			"The format is {hostname}/{owner}/{repo}, ex. github.com/runstarship/starship. '*' matches any characters until the next comma. Examples: " +
			"all repos: '*' (not secure), an entire hostname: 'internalgithub.com/*' or an organization: 'github.com/runstarship/*'." +
	...
	TFETokenFlag: {
		description: "API token for Terraform Cloud/Enterprise. This will be used to generate a ~/.terraformrc file." +
			...
			" Should be specified via the starship_TFE_TOKEN environment variable for security.",
	},
	DefaultTFVersionFlag: {
		description: "Terraform version to default to (ex. v0.12.0). Will download if not yet on disk." +
			" If not set, starship uses the terraform binary in its PATH.",
	},
	VCSStatusName: {
		description:  "Name used to identify starship for pull request statuses.",
		defaultValue: DefaultVCSStatusName,
	},
	WebUsernameFlag: {
		description:  "Username used for Web Basic Authentication on starship HTTP Middleware",
		defaultValue: DefaultWebUsername,
	},
	WebPasswordFlag: {
		description:  "Password used for Web Basic Authentication on starship HTTP Middleware",
...
var boolFlags = map[string]boolFlag{
	AllowForkPRsFlag: {
		description:  "Allow starship to run on pull requests from forks. A security issue for public repos.",
		defaultValue: false,
	},
	AllowRepoConfigFlag: {
		description: "Allow repositories to use starship.yaml files to customize the commands starship runs." +
			" Should only be enabled in a trusted environment since it enables a pull request to run arbitrary commands" +
			" on the starship server.",
		defaultValue: false,
		hidden:       true,
	},
	AutomergeFlag: {
	...
	DisableApplyAllFlag: {
		description:  "Disable \"starship apply\" command without any flags (i.e. apply all). A specific project/workspace/directory has to be specified for applies.",
		defaultValue: false,
	},
	DisableApplyFlag: {
		description:  "Disable all \"starship apply\" command regardless of which flags are passed with it.",
		defaultValue: false,
	},
	DisableAutoplanFlag: {
		description:  "Disable starship auto planning feature",
		defaultValue: false,
	},
	DisableRepoLockingFlag: {
		description: "Disable starship locking repos",
	},
	EnablePolicyChecksFlag: {
		description:  "Enable starship to run user defined policy checks.  This is explicitly disabled for TFE/TFC backends since plan files are inaccessible.",
		defaultValue: false,
	},
	EnableRegExpCmdFlag: {
		description:  "Enable starship to use regular expressions on plan/apply commands when \"-p\" flag is passed with it.",
		defaultValue: false,
	},
	EnableDiffMarkdownFormat: {
		description:  "Enable starship to format Terraform plan output into a markdown-diff friendly format for color-coding purposes.",
		defaultValue: false,
	},
	AllowDraftPRs: {
...
// NewServer returns the real starship server object.
...
func (s *ServerCmd) Init() *cobra.Command {
	c := &cobra.Command{
		Use:           "server",
		Short:         "Start the starship server",
		Long:          `Start the starship server and listen for webhook calls.`,
		SilenceErrors: true,
		SilenceUsage:  true,
	...
	// Configure viper to accept env vars prefixed with starship_ that can be
...
// setstarshipURL sets the externally accessible URL for starship.

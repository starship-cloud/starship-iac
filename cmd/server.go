...
func (s *ServerCmd) validate(userConfig server.UserConfig) error {
	...
	if userConfig.BitbucketBaseURL == "" && userConfig.BitbucketWebhookSecret != "" {
...
func (s *ServerCmd) securityWarnings(userConfig *server.UserConfig) {
	...
	if userConfig.BitbucketUser != "" && userConfig.BitbucketBaseURL != "" && userConfig.BitbucketWebhookSecret == "" && !s.SilenceOutput {
		s.Logger.Warn("no Bitbucket webhook secret set. This could allow attackers to spoof requests from Bitbucket")
	}
	if userConfig.BitbucketUser != "" && userConfig.BitbucketBaseURL == "" && !s.SilenceOutput {
